...
# ID del archivo de Word en Google Drive
document_id = '1HMyF7K9QYl0IKvpWxb8EdT7CHdiG0Oiw3J2HYOowbFQ'  # ID correcto
...
# Verifica si el archivo ya existe
if path.exists(doc):
    print(f'El archivo "{doc}" ya se encuentra descargado.')
else:
    # Si no existe, intenta descargar el archivo
    try:
        wget.download(url, doc)
        print(f'\nArchivo descargado: {doc}')
...
base_limpia_matriz['variable_mat_sup'] = variable_mat_sup
# Para cada variable bacteriana
for variable in variables_bacterianas:
    plt.figure(figsize=(8, 5))

    # Agrupar los datos por 'mat_susp' y calcular la media de la variable bacteriana
    data_agrupada = base_limpia_matriz.groupby('variable_mat_sup')[variable].mean()
